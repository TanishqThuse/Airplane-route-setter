...
import { Separator } from "@/components/ui/separator"; // Assuming you need Separator later
...
const Index = () => {
  ...
  const handleDataImport = (importedCities: City[], importedFlights: Flight[]) => {
    setCities(importedCities);
    setFlights(importedFlights);
    
    // Reset search results upon importing new data
  ...
  const handleSearch = (source: string, destination: string, priority: OptimizationPriority) => {
    ...
    // Clear previous search result visualization
    setSearchResult(null); 
    ...
    if (result) {
      const priorityText = priority === 'cost' ? 'cost-effective' : priority === 'time' ? 'fastest' : 'shortest';
      toast.success(`Optimal ${priorityText} route found!`);
      setShowOptimalPath(true); // Automatically show the path on successful search
    } else {
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="w-full lg:w-2/5 overflow-y-auto border-r border-border bg-background">
          <div className="p-4 space-y-6">
            ...
            <DataImporter onImport={handleDataImport} />
            <Separator />
              ...
              <div className="text-center py-12 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">
                    Add cities and flights manually, or use the **Import Raw Data** button above!
                  </p>
                </div>
              </div>
            )}
